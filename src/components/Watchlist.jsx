...
const Watchlist = () => {
  ...
  useEffect(() => {
    const genres = watchlist.map((movie) => genreids[movie.genre_ids[0]]);
    const uniqueGenres = new Set(genres);
    setGenreList(["All", ...uniqueGenres]);
  }, [watchlist]);

  const handleAscendingRatings = () => {
    const sortedAscending = [...watchlist].sort((m1, m2) => m1.vote_average - m2.vote_average);
    setWatchlist(sortedAscending);
  };

  const handleDescendingRatings = () => {
    const sortedDescending = [...watchlist].sort((m1, m2) => m2.vote_average - m1.vote_average);
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5">
        <table className="w-full border-collapse bg-white text-left text-sm text-gray-500">
          <thead>
            <tr className="bg-gray-50">
              ...
              <th>
                <div className="flex">
                  <div>
                    <i className="fa-solid fa-arrow-up" onClick={handleDescendingRatings}></i>{" "}
                    Ratings{" "}
                    <i className="fa-solid fa-arrow-down" onClick={handleAscendingRatings}></i>
                  </div>
                </div>
              </th>
              <th>
          ...
          <tbody className="divide-y divide-gray-100 border-t border-gray-100">
            {watchlist
              .filter((movie) => (currentGenre === "All" ? true : genreids[movie.genre_ids[0]] === currentGenre))
